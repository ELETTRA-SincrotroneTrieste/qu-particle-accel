...
unix:!android-g++ {
    ...
    PKGCONFIG += cumbia cumbia-qtcontrols qu-particle-accel
}
...
!isEmpty(INSTALL_DIR) {
    ...
    message("       execute `make install` to install 'storage-ring' under $${INSTALL_DIR} ")
    message("-")
} else {
    message("-")
    message("INSTALLATION")
    message("       call qmake INSTALL_DIR=/usr/local/bin to install storage-ring later with `make install` ")
    message("-")
}
